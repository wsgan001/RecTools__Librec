...
public abstract class AbstractRecommender implements Recommender {
    ...
    public void recommend(RecommenderContext context) throws LibrecException {
        ...
        //this.recommendedList_train = recommend("train");
        LOG.info("Job End.");
        cleanup();
    }
...
//            LOG.error("Loss = NaN or Infinity: current settings does not fit the recommender! Change the settings and try again!");
    ...
    protected void Evaluate_iter(int iter) throws LibrecException{
        ...
        if (isRanking)
        {
            ...
            if (recommendedList_iter_validate != null)
                recommendedList_iter_test = recommendedList_iter_validate;
            else
        ...
        if (eval_test != null && eval_test.size() > 0) {
            for (Map.Entry<Measure.MeasureValue, Double> entry : eval_test.entrySet()) {
                String evalName = null;
                if (entry != null && entry.getKey() != null) {
                    Double validate_score = -1.0, test_score = entry.getValue(), train_score = -1.0;
                    if (eval_validate != null)
                        validate_score = eval_validate.get(entry.getKey());
                    if (eval_train != null)
                        train_score = eval_train.get(entry.getKey());
    ...
    protected RecommendedList recommendRank_iter(String type) throws LibrecException {
        RecommendedList rl = new RecommendedItemList(numUsers - 1, numUsers);
        for (int userIdx = 0; userIdx < numUsers; ++userIdx) {
            Set<Integer> itemSet = trainMatrix.getColumnsSet(userIdx);
            for (int itemIdx = 0; itemIdx < numItems; ++itemIdx) {
            ...
            rl.topNRankItemsByUser(userIdx, topN);
        }
