...
public class RatioDataSplitter extends AbstractDataSplitter {
    ...
    private HashBasedTable<Integer,Integer,String> splitPredefinedTable;
    ...
    public void splitData() throws LibrecException {
        ...
        this.splitPredefinedTable = dataConvertor.getSplitPredefinedTable();
    ...
    /**
     * Split the ratings (by predefined) into two parts: (ratio) training, (1-ratio)
     * test subsets.
     */
    public void getRatioBySplitPredefined() {
        testMatrix = new SparseMatrix(preferenceMatrix);
        trainMatrix = new SparseMatrix(preferenceMatrix);
        validationMatrix = new SparseMatrix(preferenceMatrix);

        for (Table.Cell<Integer, Integer, String> en : splitPredefinedTable.cellSet()) {
            int row = en.getRowKey();
            int col = en.getColumnKey();
            String val = en.getValue();
            if (val.equals("Train")){
                testMatrix.set(row,col,0.0);
                validationMatrix.set(row,col,0.0);
            }
            else if (val.equals("Validate")){
                trainMatrix.set(row,col,0.0);
                testMatrix.set(row,col,0.0);
            }
            else{
                trainMatrix.set(row,col,0.0);
